...
const SimulationComparisonPanel: React.FC<SimulationComparisonPanelProps> = ({
  funds,
  montanteIniziale,
  ...
  const fundResults: FundSimResult[] = useMemo(() => {
    return funds.map((fund, idx) => {
      ...
      const imposta = montanteFinale * aliquotaSostitutiva;
      const montanteNetto = montanteFinale - imposta;
  ...
  const bestFund = useMemo(() => {
    ...
    return fundResults.reduce((best, r) => (r.montanteNetto > best.montanteNetto ? r : best), fundResults[0]);
  }, [fundResults]);
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex items-start gap-4">
        <span className="flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-violet-600 to-blue-600 text-white flex items-center justify-center text-sm sm:text-base font-bold">
          ⚡
        </span>
        <div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800/60 p-5 sm:p-6">
        <div className="mb-4">
          <h4 className="text-sm sm:text-base font-medium text-slate-700 dark:text-slate-300">
            Crescita del capitale a confronto
          </h4>
          <p className="text-xs sm:text-sm text-slate-400 dark:text-slate-500 mt-1">
            Include reinvestimento del risparmio fiscale. La linea tratteggiata è il TFR in azienda.
          </p>
        </div>
      ...
      {/* ── Per-fund result cards ───────────────────────────── */}
      <div className={`grid gap-4 ${funds.length === 2 ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-3'}`}>
        {fundResults.map((r) => {
          ...
          return (
            ...
            >
              ...
              {/* Metrics — matching single-mode layout */}
              <div className="grid grid-cols-1 gap-3">
                {/* Totale versato */}
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800/60 p-3 sm:p-4 text-center">
                  <p className="text-[10px] sm:text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Totale versato</p>
                  <p className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100">{formatCurrency(r.totaleVersato)}</p>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">capitale iniziale + volontario + TFR</p>
                </div>
                {/* Capitale accumulato */}
                <div className="rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/20 p-3 sm:p-4 text-center">
                  <p className="text-[10px] sm:text-xs font-medium text-emerald-600 dark:text-emerald-400 mb-1">Capitale accumulato</p>
                  <p className="text-base sm:text-lg font-bold text-emerald-700 dark:text-emerald-300">{formatCurrency(r.montanteFinale)}</p>
                  <p className="text-[10px] text-emerald-500 dark:text-emerald-400 mt-0.5">versato + rendimenti</p>
                </div>
                {/* Guadagno dai rendimenti */}
                <div className="rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-950/20 p-3 sm:p-4 text-center">
                  <p className="text-[10px] sm:text-xs font-medium text-blue-600 dark:text-blue-400 mb-1">Guadagno dai rendimenti</p>
                  <p className="text-base sm:text-lg font-bold text-blue-700 dark:text-blue-300">+{formatPercentage(r.rendimentoPercentuale, 1)}</p>
                  <p className="text-xs text-blue-500 dark:text-blue-400 mt-0.5">{formatCurrency(r.guadagnoRendimenti)}</p>
                </div>
              </div>
            </div>
          );
        ...
        <div className="rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20 p-5 sm:p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {(() => {
                const worst = fundResults.reduce((w, r) => (r.montanteNetto < w.montanteNetto ? r : w), fundResults[0]);
                const delta = bestFund.montanteNetto - worst.montanteNetto;
                return (

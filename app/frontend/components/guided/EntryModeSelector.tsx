...
const EntryCard: React.FC<EntryCardProps> = ({ title, description, active, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className={`w-full rounded-2xl border px-4 py-5 sm:px-5 sm:py-6 text-left transition hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus-visible:ring dark:border-slate-800 ${
      active
  ...
  >
    ...
    <p className="mt-2 sm:mt-3 text-sm sm:text-base text-slate-600 dark:text-slate-300">{description}</p>
  </button>
);
